...
def main(datasetPath, 
    ...
    print("\nCOLUMN GENERATION TIMING")
    print("="*70)
    print(f"Root CG iterations = {bp.root_cg_iterations}")
    print(f"Root RMP total = {bp.root_rmp_time:.3f} seconds")
    print(f"Root Pricing total = {bp.root_pp_time:.3f} seconds")
    print(f"Root CG wall time = {bp.root_cg_time:.3f} seconds")
    print(f"All-node RMP total = {bp.total_rmp_time:.3f} seconds")
    print(f"All-node Pricing total = {bp.total_pp_time:.3f} seconds")
    print(f"All-node CG wall time = {bp.total_cg_time:.3f} seconds")
    print("="*70)
    ...
    summary = {
        ...
        "heuristic": heuristic,
        "expanded_pricing": bool(use_expanded_pricing),
        "solve_time_sec": float(sol_time),
        "bb_nodes_processed": int(bp.bb_node_count),
        "upper_bound": float(bp.upperbound),
        "lower_bound": float(bp.lowerbound),
        "root_cg_iterations": int(bp.root_cg_iterations),
        "root_rmp_time_sec": float(bp.root_rmp_time),
        "root_pricing_time_sec": float(bp.root_pp_time),
        "root_cg_time_sec": float(bp.root_cg_time),
        "all_nodes_rmp_time_sec": float(bp.total_rmp_time),
        "all_nodes_pricing_time_sec": float(bp.total_pp_time),
        "all_nodes_cg_time_sec": float(bp.total_cg_time),
    }
...
def run_batch_experiment(instance_paths,
                         output_csv="mlpricing_timing_results.csv",
                         heuristic='savings',
                         time_limit=None,
                         use_expanded_pricing=True,
                         expanded_max_m=None,
                         expanded_max_routes=20,
                         reduced_graph_min_columns=5):
    """Run MLPricing on multiple instances and export timing summaries to CSV."""
    results = []
    for idx, dataset_path in enumerate(instance_paths, start=1):
        print("\n" + "#" * 70)
        print(f"BATCH RUN {idx}/{len(instance_paths)}: {dataset_path}")
        print("#" * 70)
        try:
            run_summary = main(
            ...
            print(f"[Batch] Failed on instance {dataset_path}: {exc}")
            results.append({
                ...
                "expanded_pricing": bool(use_expanded_pricing),
                "solve_time_sec": -1.0,
                "bb_nodes_processed": -1,
                "upper_bound": -1.0,
                "lower_bound": -1.0,
                "root_cg_iterations": -1,
                "root_rmp_time_sec": -1.0,
                "root_pricing_time_sec": -1.0,
                "root_cg_time_sec": -1.0,
                "all_nodes_rmp_time_sec": -1.0,
                "all_nodes_pricing_time_sec": -1.0,
                "all_nodes_cg_time_sec": -1.0,
                "status": "error",
                "error": str(exc),
            })
    ...
    fieldnames = [
        ...
        "root_cg_iterations",
        "root_rmp_time_sec",
        "root_pricing_time_sec",
        "root_cg_time_sec",
        "all_nodes_rmp_time_sec",
        "all_nodes_pricing_time_sec",
        "all_nodes_cg_time_sec",
        "status",
        "error",
    ]
    ...
    print(f"\n[Batch] Timing results written to: {output_csv}")
    return results


def _collect_instances(folder=None, instances=None):
    paths = []
    if instances:
        paths.extend(instances)
    if folder:
        txt_files = [
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run MLPricing on one or many instances and log timing stats.")
    parser.add_argument("--dataset", default="C:/Users/CiSTUP/Downloads/CVRTPW_Dataset/solomon_25_customer_instances/r101.txt",
                        help="Single instance path (.txt). Used when no --folder/--instances are given.")
    parser.add_argument("--folder", default=None,
                        help="Folder containing .txt instances for batch experiment.")
    parser.add_argument("--instances", nargs="*", default=None,
                        help="Explicit instance paths for batch experiment.")
    parser.add_argument("--output-csv", default="mlpricing_timing_results.csv",
                        help="CSV output path for batch timing results.")
    parser.add_argument("--heuristic", default="savings", choices=["savings", "solomon"],
                        help="Initial route heuristic.")
    parser.add_argument("--time-limit", type=float, default=1000.0,
                        help="Time limit in seconds for each instance.")
    parser.add_argument("--showfig", action="store_true", help="Enable visualization in single-instance mode.")
    parser.add_argument("--disable-expanded-pricing", action="store_true",
                        help="Disable expanded-state pricing backend.")
    parser.add_argument("--expanded-max-m", type=int, default=None,
                        help="Optional cap on expanded graph states.")
    parser.add_argument("--expanded-max-routes", type=int, default=20,
                        help="Max routes generated per expanded pricing iteration.")
    parser.add_argument("--reduced-graph-min-columns", type=int, default=5,
                        help="Reduced expanded graph fallback threshold.")
    args = parser.parse_args()

    use_expanded = not args.disable_expanded_pricing
    batch_instances = _collect_instances(folder=args.folder, instances=args.instances)

    if batch_instances:
        run_batch_experiment(
            instance_paths=batch_instances,
            output_csv=args.output_csv,
            heuristic=args.heuristic,
            time_limit=args.time_limit,
            use_expanded_pricing=use_expanded,
            expanded_max_m=args.expanded_max_m,
            expanded_max_routes=args.expanded_max_routes,
            reduced_graph_min_columns=args.reduced_graph_min_columns,
        )
    else:
        main(
