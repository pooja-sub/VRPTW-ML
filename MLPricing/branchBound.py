...
class BranchAndBound:
    ...
    def bb_node(self, user_param, routes, branching, best_routes, depth):
        ...
        cg_time = _time.time() - cg_start
        # If this is the root node, print LP relaxation time separately
        if getattr(branching, 'toplevel', False):
            print(f"[BranchAndBound] Root LP relaxation time = {cg_time:.3f}s")
