...
def graphTransformation_digraph(G_original, m=1):
    """
    Build an expanded graph from a generic directed graph using org-style rules.
...
class GraphTransformation:
    ...
    # def _filter_infeasible_paths(self, destination, reachable_nodes):
    #     """
    #     Generate all feasible subsets (paths) from reachable nodes to destination.
    #     Enumerates feasible paths by generating subsets up to a reasonable size.
        
    #     A path (subset S) is feasible if:
    #     1. Total demand of S ≤ vehicle capacity
    #     2. Route depot -> S -> destination is time-window feasible
        
    #     Args:
    #         destination: Target node
    #         reachable_nodes: Set of nodes that can reach destination
            
    #     Returns:
    #         feasible_paths: Set of feasible subsets (each subset is a frozenset/tuple)
    #     """
    ...
    #     # Generate subsets: empty set first (no intermediate customers)
    #     empty_path = frozenset()
    #     if self._is_path_to_destination_feasible(destination, set(), depot):
    #         feasible_paths.add(empty_path)
        
    #     # Then 1-node, 2-node, 3-node paths, etc. (limit to avoid exponential blowup)
    #     max_path_length = min(len(reachable_nodes), 3)  # Limit to paths of at most 3 nodes for performance
        
    #     for path_length in range(1, max_path_length + 1):
    #         for path_nodes in combinations(sorted(reachable_nodes), path_length):
    #             path_set = set(path_nodes)
                
    #             # Check capacity
    #             total_demand = sum(self.params.d[n] for n in path_set)
    #             if total_demand > self.params.capacity:
    #                 continue
                
    #             # Check time window feasibility
    #             if self._is_path_to_destination_feasible(destination, path_set, depot):
    #                 feasible_paths.add(frozenset(path_set))
        
    #     return feasible_paths
    
    # def _is_path_to_destination_feasible(self, destination, intermediate_nodes, start_node):
    #     """
    #     Check if a path visiting intermediate_nodes and ending at destination is feasible.
    #     Path is: start_node -> visit(intermediate_nodes in optimal order) -> destination
        
    #     Uses greedy nearest-neighbor to find best visitation order.
        
    #     Args:
    #         destination: Final destination node
    #         intermediate_nodes: Set of nodes to visit before destination
    #         start_node: Starting node (usually depot)
            
    #     Returns:
    #         True if feasible path exists, False otherwise
    #     """
    #     depot = start_node
    #     verybig = self.params.verybig
        
    #     # If no intermediate nodes, just check if direct path exists
    #     if not intermediate_nodes:
    #         if self.params.dist[depot][destination] >= verybig - 1e-6:
    #             return False
    #         arrival_time = self.params.a[depot] + self.params.ttime[depot][destination]
    #         return arrival_time <= self.params.b[destination]
    ...
    #             if self.params.dist[current_node][next_node] >= verybig - 1e-6:
    #                 continue
    ...
    #             return False  # No feasible continuation
    ...
    #     # Now check final arc to destination
    #     if self.params.dist[current_node][destination] >= verybig - 1e-6:
    #         return False
        
    #     final_arrival_time = current_time + self.params.ttime[current_node][destination]
    #     return final_arrival_time <= self.params.b[destination]
    ...
    # def _is_edge_feasible(self, k, l):
    #     """
    #     Check if an edge between two nodes is feasible.
        
    #     Combines the two paths and checks feasibility of the combined path.
        
    #     Args:
    #         k: First node (frozenset of customers)
    #         l: Second node (frozenset of customers)
            
    #     Returns:
    #         True if combined path is feasible, False otherwise
    #     """
    #     if not k or not l:  # If either is empty, skip for now
    #         return False
        
    #     depot = 0
    #     capacity = self.params.capacity
        
    #     # Combine paths (avoiding duplicates) - union of two frozensets
    #     combined_frozenset = k | l
    #     # Immediately convert to mutable set for iteration and modification
    #     combined_set = set(combined_frozenset)
        
    #     # Check 1: Total demand
    #     total_demand = sum(self.params.d[node] for node in combined_set)
    #     if total_demand > capacity:
    #         return False
        
    #     # Check 2: Time window feasibility along combined path
    #     # Use greedy nearest-neighbor ordering - duplicate the set for iteration
    #     current_time = self.params.a[depot]
    #     current_node = depot
    #     remaining = set(combined_set)  # Create mutable copy for discard operations
    ...
    #         # Find nearest unvisited node
    #         for next_node in list(remaining):  # Convert to list to avoid mutation during iteration
    #             if self.params.dist[current_node][next_node] >= self.params.verybig - 1e-6:
    #                 continue
    ...
    def visualize_phase1_graph(self, instance_name=None, save_path=None):
        """
        Visualize the Phase 1 original graph with node positions.
        
        Args:
            instance_name: Name of the instance to display in title
            save_path: Optional path to save the figure
        """
        if self.graph is None:
            print("[Error] Phase 1 graph not created yet")
            return
        
        plt.figure(figsize=(12, 10))
        
        # Use actual coordinates from the Solomon instance
        pos = {}
        for node in self.graph.nodes():
            pos[node] = (self.params.posx[node], self.params.posy[node])
        
        # Draw graph
        nx.draw_networkx_edges(self.graph, pos, alpha=0.3, width=0.5)
        
        # Draw nodes with different colors for depot vs customers
        depot_nodes = [0]
        customer_nodes = [n for n in self.graph.nodes() if n != 0]
        
        nx.draw_networkx_nodes(self.graph, pos, nodelist=depot_nodes, 
                              node_color='red', node_size=500, label='Depot')
        nx.draw_networkx_nodes(self.graph, pos, nodelist=customer_nodes, 
                              node_color='lightblue', node_size=300, label='Customers')
        
        # Draw labels
        nx.draw_networkx_labels(self.graph, pos, font_size=8, font_weight='bold')
        
        title = "Phase 1: Original VRPTW Network"
        if instance_name:
        ...
        if save_path:
            ...
            print(f"[Phase 1 graph saved to {save_path}]")
        
        plt.close()  # Close without displaying
    
    def visualize_phase2_graph(self, instance_name=None, save_path=None):
        """
        Visualize the Phase 2 expanded graph using spring layout.
        Edges are made more visible with increased width and opacity.
        ...
        if self.expanded_graph is None:
            print("[Error] Phase 2 expanded graph not created yet")
            return
        
        G = self.expanded_graph
        print(f"[Visualizing Phase 2 graph with {G.number_of_nodes()} nodes and {G.number_of_edges()} edges]")
        print("[Computing layout positions - this may take a moment...]")
        
        # Convert MultiDiGraph to DiGraph for layout computation
        G_simple = nx.DiGraph(G)
        
        # Use spring layout for better visualization of dense graphs
        pos = nx.spring_layout(G_simple, k=0.5, iterations=50, seed=42)
        
        plt.figure(figsize=(14, 12))
        
        # Draw edges with improved visibility
        nx.draw_networkx_edges(G_simple, pos, alpha=0.4, width=1.2, 
                              edge_color='steelblue', arrows=True, arrowsize=8,
                              arrowstyle='-|>', connectionstyle='arc3,rad=0.1')
        
        # Draw nodes with uniform size - color by degree
        ...
        cbar = plt.colorbar(nodes, label='Node Degree')
        
        # Draw labels only if graph is small enough
        if G.number_of_nodes() < 100:
            nx.draw_networkx_labels(G_simple, pos, font_size=6, font_weight='bold')
        
        title = f"Phase 2: Expanded Network ({G.number_of_nodes()} nodes, {G.number_of_edges()} edges)"
        if instance_name:
        ...
        if save_path:
            plt.savefig(save_path, dpi=150, bbox_inches='tight')
            print(f"[Phase 2 graph saved to {save_path}]")
...
if __name__ == "__main__":
    ...
    # Configuration for different instance sizes
    instance_sizes = {
        # 25: [
        #     "C:\\Users\\CiSTUP\\Downloads\\VRTPW_Dataset\\solomon_25_customer_instances\\c101.txt",
        #     "C:\\Users\\CiSTUP\\Downloads\\VRTPW_Dataset\\solomon_25_customer_instances\\r101.txt",
        #     "C:\\Users\\CiSTUP\\Downloads\\VRTPW_Dataset\\solomon_25_customer_instances\\rc101.txt"
        # ],
        50: [
            "C:\\Users\\CiSTUP\\Downloads\\VRTPW_Dataset\\solomon_50_customer_instances\\c101.txt",
            "C:\\Users\\CiSTUP\\Downloads\\VRTPW_Dataset\\solomon_50_customer_instances\\r101.txt",
            "C:\\Users\\CiSTUP\\Downloads\\VRTPW_Dataset\\solomon_50_customer_instances\\rc101.txt"
        ],
        # 100: [
        #     "C:\\Users\\CiSTUP\\Downloads\\VRTPW_Dataset\\solomon_100_customer_instances\\c101.txt",
        #     "C:\\Users\\CiSTUP\\Downloads\\VRTPW_Dataset\\solomon_100_customer_instances\\r101.txt",
        #     "C:\\Users\\CiSTUP\\Downloads\\VRTPW_Dataset\\solomon_100_customer_instances\\rc101.txt"
        # ],
        # 200: [
        #     "C:\\Users\\CiSTUP\\Downloads\\VRTPW_Dataset\\homberger_200_customer_instances\\C1_2_1.txt",
        #     "C:\\Users\\CiSTUP\\Downloads\\VRTPW_Dataset\\homberger_200_customer_instances\\R1_2_1.txt",
        #     "C:\\Users\\CiSTUP\\Downloads\\VRTPW_Dataset\\homberger_200_customer_instances\\RC1_2_1.txt"
        # ],
        # 400: [
        #     "C:\\Users\\CiSTUP\\Downloads\\VRTPW_Dataset\\homberger_400_customer_instances\\C1_4_1.txt",
        #     "C:\\Users\\CiSTUP\\Downloads\\VRTPW_Dataset\\homberger_400_customer_instances\\R1_4_1.txt",
        #     "C:\\Users\\CiSTUP\\Downloads\\VRTPW_Dataset\\homberger_400_customer_instances\\RC1_4_1.txt"
        # ]
    }
    
    # Configuration: m value(s) to test
    m_values = [4]  # List of m values to test
    
    # Run for each m value
    for m_value in m_values:
        print("\n" + "#"*70)
        print(f"# RUNNING EXPERIMENTS WITH m={m_value} PREDECESSORS")
        print("#"*70)
        
        # Create output directories with m suffix
        output_dir = f"graph_visualizations_m{m_value}"
        metrics_dir = f"graph_metrics_m_{m_value}"
        ...
        available_instances = []
        print("\n" + "="*70)
        print("SCANNING FOR AVAILABLE INSTANCES")
        print("="*70)
        
        for size, instances in instance_sizes.items():
            found_count = 0
            for instance_path in instances:
                if os.path.exists(instance_path):
                    available_instances.append(instance_path)
                    found_count += 1
                    print(f"[Found {size}-customer instance: {os.path.basename(instance_path)}]")
            if found_count == 0:
                print(f"[WARNING: No {size}-customer instances found at expected locations]")
        
        print(f"\n[Total available instances: {len(available_instances)}]")
        
        if len(available_instances) == 0:
            print("[ERROR: No instances found. Check paths in instance_sizes configuration]")
            continue  # Skip to next m value
        
        # Summary statistics
        results_summary = []
        
        # Process each instance
        for idx, instance_path in enumerate(available_instances, 1):
            instance_name = os.path.splitext(os.path.basename(instance_path))[0].upper()
            
            # Extract customer count from path
            if "25_customer" in instance_path:
                n_customers = 25
            elif "50_customer" in instance_path:
                n_customers = 50
            elif "100_customer" in instance_path:
                n_customers = 100
            elif "200_customer" in instance_path:
                n_customers = 200
            elif "400_customer" in instance_path:
                n_customers = 400
            else:
                n_customers = "Unknown"
            
            # Create metrics file for this instance
            metrics_file_path = os.path.join(metrics_dir, f"metrics_{instance_name}_{n_customers}.txt")
            
            try:
                print("\n" + "="*70)
                print(f"[{idx}/{len(available_instances)}] PROCESSING: {instance_name} ({n_customers} customers)")
                print("="*70)
                
                gt = GraphTransformation(instance_path, m=m_value)
                
                # Open metrics file
                gt.metrics_file = open(metrics_file_path, 'w')
                gt._log_metrics("="*70)
                gt._log_metrics(f"INSTANCE: {instance_name} ({n_customers} customers)")
                gt._log_metrics("="*70)
                
                # Phase 1: Enumerate feasible vectors of visited nodes
                print("\n" + "="*60)
                print("RUNNING PHASE 1")
                print("="*60)
                gt._log_metrics("\n" + "="*60)
                gt._log_metrics("RUNNING PHASE 1")
                gt._log_metrics("="*60)
                
                gt.create_undirected_graph()
                M = gt.enumerate_phase1()
                # gt.print_summary()
                
                # Phase 1: Degree Statistics
                print("\n" + "="*60)
                print("PHASE 1 DEGREE STATISTICS")
                print("="*60)
                gt._log_metrics("\n" + "="*60)
                gt._log_metrics("PHASE 1 DEGREE STATISTICS")
                gt._log_metrics("="*60)
                gt.compute_degree_stats("Phase 1 Graph (Original Network)", gt.graph)
                
                # Phase 2: Create expanded graph
                print("\n" + "="*60)
                print("RUNNING PHASE 2")
                print("="*60)
                gt._log_metrics("\n" + "="*60)
                gt._log_metrics("RUNNING PHASE 2")
                gt._log_metrics("="*60)
                expanded_graph = gt.enumerate_phase2()
                
                # Phase 2: Degree Statistics
                print("\n" + "="*60)
                print("PHASE 2 DEGREE STATISTICS")
                print("="*60)
                gt._log_metrics("\n" + "="*60)
                gt._log_metrics("PHASE 2 DEGREE STATISTICS")
                gt._log_metrics("="*60)
                gt.compute_degree_stats("Phase 2 Graph (Expanded Network)", gt.expanded_graph)
                
                # Compare graph sparsity
                gt.compare_graphs()
                ...
                results_summary.append({
                    ...
                    'phase1_nodes': gt.graph.number_of_nodes(),
                    'phase1_edges': gt.graph.number_of_edges(),
                    'phase2_nodes': gt.expanded_graph.number_of_nodes(),
                    'phase2_edges': gt.expanded_graph.number_of_edges(),
                    'status': 'SUCCESS'
                })
                
                # Skip visualizations to save time
                print(f"\n[Skipping visualizations for {instance_name}]")
                gt._log_metrics(f"\n[Skipping visualizations for {instance_name}]")
                
                # Close metrics file
                if gt.metrics_file:
                    gt._log_metrics(f"\n[Metrics saved to {metrics_file_path}]")
                    gt.metrics_file.close()
                
            except FileNotFoundError as e:
                print(f"[ERROR] File not found for {instance_name}: {e}")
                results_summary.append({
                    'name': instance_name,
                    'customers': n_customers,
                    'status': f'FILE_NOT_FOUND: {e}'
                })
                continue
            except Exception as e:
                print(f"[ERROR] Error processing {instance_name}: {e}")
                import traceback
                traceback.print_exc()
                results_summary.append({
                    'name': instance_name,
    ...
    with open(summary_file_path, 'w') as f:
        for result in results_summary:
            print(f"\n{result['name']} ({result['customers']} customers):")
            f.write(f"\n{result['name']} ({result['customers']} customers):\n")
            
            if result['status'] == 'SUCCESS':
                print(f"  Phase 1: {result['phase1_nodes']} nodes, {result['phase1_edges']} edges")
                print(f"  Phase 2: {result['phase2_nodes']} nodes, {result['phase2_edges']} edges")
                print(f"  Node expansion: {result['phase2_nodes']/result['phase1_nodes']:.2f}x")
                print(f"  Edge expansion: {result['phase2_edges']/result['phase1_edges']:.2f}x")
                
                f.write(f"  Phase 1: {result['phase1_nodes']} nodes, {result['phase1_edges']} edges\n")
                f.write(f"  Phase 2: {result['phase2_nodes']} nodes, {result['phase2_edges']} edges\n")
                f.write(f"  Node expansion: {result['phase2_nodes']/result['phase1_nodes']:.2f}x\n")
                f.write(f"  Edge expansion: {result['phase2_edges']/result['phase1_edges']:.2f}x\n")
            else:
        ...
        print(f"[Detailed metrics saved to '{metrics_dir}' directory]")
        print("\n" + "="*70)
        print(f"ALL INSTANCES PROCESSED FOR m={m_value}")
        print("="*70)
    
    print("\n" + "#"*70)
    print("# ALL m VALUES COMPLETED")
    print("#"*70)
