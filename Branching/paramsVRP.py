...
class ParamsVRP:
    ...
    def init_params(self, input_path):
        try:
            ...
            self.wval = np.zeros(self.nbclients + 2)
            ...
            # Set depot to depot distance as infinity and diagonal to infinity
            for i in range(self.nbclients + 2):
            ...
            for i in range(1, self.nbclients + 2):
                self.wval[i] = 0.0
    ...
    def preprocess_arcs(self):
        ...
        for i in range(self.nbclients + 2):
            for j in range(self.nbclients + 2):
                ...
                if i != 0 and j != self.nbclients:  # Not from start depot or to end depot
                ...
                # Note: tij already includes si (service time at i)
                if not eliminated:
                    # Earliest arrival at j from i = earliest time at i + travel time (which includes service time at i)
                    earliest_arrival_j = self.a[i] + self.ttime[i][j]
